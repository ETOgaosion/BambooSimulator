...
from simulation_livepipe_red2_20.mysimulator import MySimulator
...
def main(args):
    ...
    if not options.generate_table:
        ...
        if options.spot_instance_trace is None:
            options.fig_directory = 'res/simutest-livepipe-red2-20-prob-' + options.model_size + '-' + str(options.removal_probability)
        else:
            options.fig_directory = 'res/simutest-livepipe-red2-20-' + options.spot_instance_trace.split('/')[-1].split('-')[0] + '-' + options.model_size
        options.system_name = 'livepipe-red2-20'
        options.data_dir = 'data/livepipe-red2-20'
