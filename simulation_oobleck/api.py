...
def main(args):
    ...
    if not options.generate_table:
        ...
        if options.spot_instance_trace is None:
            options.fig_directory = 'res/simutest-oobleck-prob-' + options.model_size + '-' + str(options.removal_probability)
        else:
            options.fig_directory = 'res/simutest-oobleck-' + options.spot_instance_trace.split('/')[-1].split('-')[0] + '-' + options.model_size
