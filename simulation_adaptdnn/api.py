...
from simulation_adaptdnn.mysimulator import MySimulator
...
def main(args):
    ...
    if not options.generate_table:
        ...
        if options.spot_instance_trace is None:
            options.fig_directory = 'res/simutest-adaptdnn-prob-' + options.model_size + '-' + str(options.removal_probability)
        else:
