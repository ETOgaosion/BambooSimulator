...
from simulation_oobleck_opt_20.mysimulator import MySimulator
...
def main(args):
    ...
    if not options.generate_table:
        ...
        if options.spot_instance_trace is None:
            options.fig_directory = 'res/simutest-oobleck-opt-20-prob-' + options.model_size + '-' + str(options.removal_probability)
        else:
            options.fig_directory = 'res/simutest-oobleck-opt-20-' + options.spot_instance_trace.split('/')[-1].split('-')[0] + '-' + options.model_size
        options.system_name = 'oobleck-opt-20'
        options.data_dir = 'data/oobleck-opt-20'
